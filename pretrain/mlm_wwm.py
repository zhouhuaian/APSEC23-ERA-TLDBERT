...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if "validation" not in datasets.keys():
            train_val_ds = datasets["train"].train_test_split(test_size=0.01, shuffle=True, seed=42)
    ...
    # offline_files_dir = "../offline_files/" + model_args.model_name_or_path.split('/')[-1]

    # config.save_pretrained(offline_files_dir)
    ...
    # tokenizer.save_pretrained(offline_files_dir)
    ...
    # model.save_pretrained(offline_files_dir)
    ...
    if training_args.do_eval:
        ...
        eval_output["eval_samples"] = len(trainer.eval_dataset)
        try:
